...
INT CScreen::GetImageByteSize()
{
	return sizeof(BITMAPFILEHEADER) + 54 + dwLen_2;
}

BOOL CScreen::WriteToBuf(LPSTR buf, INT imgSize)
{
	if (!buf || (imgSize < GetImageByteSize()))
	{
		return FALSE;
	}
	BITMAPFILEHEADER bfh;
	...
	memcpy(buf + sizeof(BITMAPFILEHEADER), hDib, bfh.bfSize);
	return TRUE;
}
